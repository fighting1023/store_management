layui.define(['layer', 'laytpl', 'element'], function(exports) {
     ...
     */
        // 渲染侧边栏        
         function getadmin(){
           $('.layui-side .layui-side-scroll ul').append(
                '<li class="lau-nav-item">'+
                   ' <a class="lau-nav-header cl" lau-href="./html/preview.html"><i class="layui-icon layui-icon-home"></i><cite>首页</cite></a>'+
                '</li>'+
                '<li class="lau-nav-item lau-open">'+
                    '<a class="lau-nav-header"><i class="layui-icon layui-icon-right"></i><cite>出入库</cite>'+
                    '</a>'+
                    '<dl class="lau-nav-child">'+
                        '<dd>'+
                            '<a lau-href="./html/hqstock.html"><i class="layui-icon layui-icon-circle"></i><cite>总部出入库</cite></a>'+
                        '</dd>'+
                       ' <dd>'+
                           ' <a lau-href="./html/zkstock.html"><i class="layui-icon layui-icon-circle"></i><cite>中科院出入库</cite></a>'+
                       ' </dd>'+
                   ' </dl>'+
               ' </li> '
                    )
         ...
         function getpeople(){
           $('.layui-side .layui-side-scroll ul').append(
                '<li class="lau-nav-item lau-open">'+
                    '<a class="lau-nav-header cl"><i class="layui-icon layui-icon-right"></i><cite>出入库</cite>'+
                    '</a>'+
                    '<dl class="lau-nav-child">'+
                        '<dd>'+
                            '<a class="this_a" lau-href="./html/hqstock.html"><i class="layui-icon layui-icon-circle"></i><cite>总部出入库</cite></a>'+
                        '</dd>'+
                       ' <dd>'+
                           ' <a lau-href="./html/zkstock.html"><i class="layui-icon layui-icon-circle"></i><cite>中科院出入库</cite></a>'+
                       ' </dd>'+
        ...
        function setbar(){
           ...
           let power = $.cookie("power")
            $('#ming').text( $.cookie("username"))
           if(power==1){
              getadmin();
           }else if(power==2){
               $(document).find('.layui-body').attr('data-title','总部出入库')
               $('.layui-body').attr('data-icon','layui-icon-circle')
               $('.layui-body').attr('data-href','./html/hqstock.html')
              getpeople()
           }else{
        ...
        bodyTab: [
            ...
            '<div class="layui-tab layui-tab-brief" lay-allowClose="true" lay-filter="lau-tabs">',
                '<ul class="layui-tab-title">',
                    '<li class="layui-this" lay-id="{{ d.href }}"><i class="{{ d.icon ? (d.icon.split(\'/\s+/\').length > 1 ? d.icon : \'layui-icon \' + d.icon) : \'layui-icon layui-icon-home\' }}"></i> {{ d.title || \'概况\' }}</li>',
                '</ul>',
    ...
    //         var href=_this.attr('lay-id')
    //         $(".layui-show").find("iframe").attr('src',href)
    //     }
    // })
    ...
    var Layout = function () {
        ...
        if (SINGLE) {
            ...
            element.on('tab(' + tabFilter + ')', function(data) {
              ...
              if(LAYID=='./html/survey.html'){
                   $('.lau-nav-item').eq(0).find('a').addClass('cl');
                   $('.lau-nav-item').eq(0).siblings().find('.lau-nav-child dd a').removeClass('this_a')
                   $('.lau-nav-item').eq(0).siblings().find('.lau-nav-header').removeClass('cl')
               }else{ 
                for(var i=0;i<$(document).find('.lau-nav-item a').length;i++){
                    if(LAYID==$(document).find('.lau-nav-item a').eq(i).attr('lau-href')){
                            var _left=$(document).find('.lau-nav-item a').eq(i);
                            _left.addClass('this_a').parents('dd').siblings().find('a').removeClass('this_a');
                            _left.addClass('this_a').parents('li').siblings().find('dd a').removeClass('this_a');
                            _left.addClass('this_a').parents('li').siblings().find('.lau-nav-header').removeClass('this_a');
                            _left.parents('li').find('.lau-nav-header').addClass('cl')
                            _left.parents('li').siblings().find('.lau-nav-header').removeClass('cl')
                    }
                 }
                }
            });
        ...
        $(document).on('click', '*[lau-href]', function () {
            ...
            if(_this.attr('mark')=='m10000'){
                _this.parents('li').find('.lau-nav-header').addClass('cl')
                _this.parents('li').siblings().find('.lau-nav-header').removeClass('cl')
                _this.parents('dd').siblings().find('a').removeClass('this_a');
                _this.parents('li').siblings().find('dd a').removeClass('this_a');
                _this.parents('li').siblings().find('.lau-nav-header').removeClass('this_a');
            }else{
            _this.addClass('this_a').parents('dd').siblings().find('a').removeClass('this_a');
            _this.addClass('this_a').parents('li').siblings().find('dd a').removeClass('this_a');
            _this.addClass('this_a').parents('li').siblings().find('.lau-nav-header').removeClass('this_a');
            _this.parents('li').find('.lau-nav-header').addClass('cl')
            _this.parents('li').siblings().find('.lau-nav-header').removeClass('cl')
            }
        ...
        //监听侧栏缩进
            ...
            $(document).on('click', '.dian', function () {
                if( $(document).find('.tou .dian').attr('data')==0){
    
                     $(document).find('.dian').attr('data',1)
                    
                     $(document).find('.bars').attr('data',1)
                     $('.icons').animate({left:50},200);

                     $('.bars').animate({left:51},200);
                     $('.layui-layout-admin .layui-body').animate({left:50},200);
                     $('.layui-side').animate({width:50},200);
                    
                     $('.dian i').removeClass('layui-icon-spread-right');
                     $('.dian i').addClass('layui-icon-spread-left');
                     $(document).find('.logos').animate({width:50},200)
                     setTimeout("$(document).find('.logos img').attr('src','./static/layui/images/logoxiao.png')",210); 
                    
                }else{ 
                    $(document).find('.tou .dian').attr('data',0)
                   
                     $(document).find('.bars').attr('data',0)
                    $('.icons').animate({left:180},200);
                    $('.bars').animate({left:180},200);
                    $('.layui-layout-admin .layui-body').animate({left:180},200);
                    $('.layui-side').animate({width:180},200);             
                    $('.dian i').removeClass('layui-icon-spread-left');
                    $('.dian i').addClass('layui-icon-spread-right');
                    $(document).find('.logos').animate({width:180},200)
                    $(document).find('.logos img').attr('src','./static/layui/images/logo3s.png')
