const plugins = {
  // ajax接口统一部分
  "realm": function () {
    let realm = 'http://152.136.18.170/audit/v1'
    return realm;
  },
   // 登录页面设置
  "index": function () {
  ...
  "AjaxPlugin": function (option) {
    ...
    $.ajax({
      ...
      success: function (data) {
        // plugins.console(data)
         layer.closeAll('loading');
        if (data.code == 18000 || !data.code) {
          
          option.action(data)
        } else if (data.code == 13000) {
          window.parent.parent.location.href = plugins.index() + '/html/login.html'
    ...
    plugins.AjaxPlugin({
      ...
      action: function (data) {
        ...
        layui.use('laypage', function () {
          ...
          laypage.render({
            elem: option.elem,
            count: data.data.pageDetail.pageCount,
